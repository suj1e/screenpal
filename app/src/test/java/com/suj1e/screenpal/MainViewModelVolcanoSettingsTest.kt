...
class MainViewModelVolcanoSettingsTest {
    ...
    @Test
    fun unrelatedUpdate_preservesTranslationEnabled() {
        // Merge-regression guard: SettingsRepository.update overwrites every
        // key, so an unrelated setter must not reset translationEnabled.
        val repository = SettingsRepository(RuntimeEnvironment.getApplication())
        val viewModel = newViewModel(repository)

        viewModel.setTranslationEnabled(false)
        viewModel.setTtsVoice("BV700_streaming")

        val deadline = System.currentTimeMillis() + 5000
        var settings = runBlocking { repository.userSettings.first() }
        while (settings.ttsVoice != "BV700_streaming" && System.currentTimeMillis() < deadline) {
            Thread.sleep(20)
        ...
        assertEquals("BV700_streaming", settings.ttsVoice)

        // Self-clean: later test classes share this DataStore file and assert
        // the default translationEnabled=true.
        runBlocking { repository.update { copy(translationEnabled = true) } }
    }
}
