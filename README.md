...
- **框选识别**：点击悬浮球 → 自动截取当前屏幕 → 全屏截图上拖拽框选区域（最小 48dp）→ 自动 OCR 识别 → 自动 TTS 播报。
...
5. **识别播报**：点击悬浮球 → 系统弹出屏幕录制授权确认 → 拖拽框选要朗读的文字区域 → 松手后自动识别并播报。
6. **调整配置**：回到 App 可切换 OCR 模式、TTS 引擎、语速（0.5x–2.0x）、音调（0.5–2.0），并填写云端 API Key。
...
- Google Cloud Vision / TTS 为付费 API（各有免费额度），需自行申请 API Key。
