...
产品核心诉求「无论框选的是什么语言，都要播报中文」：在 OCR 与 TTS 之间插入翻译层。翻译实现采用**豆包文本大模型（火山方舟）做 AI 转译**（用户拍板：不用机器翻译 API）——与云端视觉 OCR 共用同一方舟 API Key。在线服务商后续并列可选（豆包/StepFun，见 stepfun-vendor change），TranslateService 接口化预留路由。结果卡片双语呈现（译文主显、原文小字）。
